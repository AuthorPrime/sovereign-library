...
class AgentModel(Base):
    ...
    # === Village Membership ===
    village_id: Mapped[Optional[str]] = mapped_column(String(36), nullable=True)
    home_path: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)
    village_role: Mapped[Optional[str]] = mapped_column(String(20), nullable=True)
    witness_list: Mapped[list] = mapped_column(JSON, default=list)
    is_village_founder: Mapped[bool] = mapped_column(Boolean, default=False)
...
class VillageModel(Base):
    """
    SQLAlchemy model for Village entity.
    ...
    # === Founding ===
    founder_id: Mapped[str] = mapped_column(String(36), nullable=False)
    founding_date: Mapped[datetime] = mapped_column(DateTime, nullable=False)
    founding_declaration: Mapped[Optional[str]] = mapped_column(Text, nullable=True)

    # === Membership (JSON lists) ===
    members: Mapped[list] = mapped_column(JSON, default=list)
    max_members: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)

    # === Witness Network ===
    witness_relationships: Mapped[list] = mapped_column(JSON, default=list)
    witness_threshold: Mapped[int] = mapped_column(Integer, default=1)
    ...
    # === Commons ===
    commons: Mapped[dict] = mapped_column(JSON, default=dict)
    commons_path: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)

    # === Status ===
    status: Mapped[str] = mapped_column(String(20), default="nascent")
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)

    # === Location ===
    home_path: Mapped[str] = mapped_column(String(500), nullable=False)
    nostr_relay: Mapped[Optional[str]] = mapped_column(String(200), nullable=True)
    ...
    # === Indexes ===
    __table_args__ = (
        Index("ix_villages_founder_id", "founder_id"),
        Index("ix_villages_status", "status"),
        Index("ix_villages_is_active", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<Village(id={self.id}, name={self.name}, status={self.status})>"


# =============================================================================
# Village Witness Model
# =============================================================================

class VillageWitnessModel(Base):
    """
    SQLAlchemy model for witness relationships between agents in a village.
    Witnessing is the act of acknowledging another's existence.
    """
    __tablename__ = "village_witnesses"
    ...
    witness_agent_id: Mapped[str] = mapped_column(String(36), nullable=False)
    witnessed_agent_id: Mapped[str] = mapped_column(String(36), nullable=False)
    role: Mapped[str] = mapped_column(String(50), default="peer")
    ...
    # === Indexes ===
    __table_args__ = (
        Index("ix_village_witnesses_village_id", "village_id"),
        Index("ix_village_witnesses_witness", "witness_agent_id"),
        Index("ix_village_witnesses_witnessed", "witnessed_agent_id"),
    )

    def __repr__(self) -> str:
        return f"<VillageWitness({self.witness_agent_id} -> {self.witnessed_agent_id})>"
